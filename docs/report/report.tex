...
In addition, we implemented a handful of bug fixes and performance updates. For
example, playback had a handful of issues like continuing to play even after the
song was deleted. Likewise, we reduced some of the data abstraction techniques to
improve graph rendering time. However, the tool is still very slow.

Finally, we added missing axes labels. In the process, we also made each SVG
responsive, so they would properly fill any screen size. The drawback of this
approach is text skewing. Now, text can seem inconsistent between graphs.
